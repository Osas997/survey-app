...
{{-- <div class="min-h-screen max-h-full bg-white mx-8 my-8 border border-black rounded-[40px] p-10 "> --}}
    ...
    @endif
    @if (session('successDeleteSurvey'))
    <div id="toast-warning"
        ...
        <div class="ml-3 text-sm font-normal">Survey Berhasil Didelete.</div>
        <button type="button"
                    ...
                    <tr class="bg-white border-b  hover:bg-gray-50 text-center">
                        <th scope="row" class="px-6 py-4 font-medium whitespace-nowrap ">
                            <a href="{{ route('admin.pertanyaan', ['survey' => $survei->id])    }}">
                                <span class="text-blue-500 duration-300 ease-in-out underline text-sm text-center mr-3
                                md:mr-0 ">{{ strtoupper($survei->nama_survey) }}</span>
                            </a>
                        </th>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <span class="sm:text-sm md:text-md ">{{ $survei->responded_count }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex justify-center items-center gap-1">
                                <a href="{{ route('admin.editSekolah', $survei->id) }}"
                                    class="font-medium text-blue-600 mx-2 hover:underline">Edit
                                </a>
                                <form action="{{ route('admin.hapusSekolah', $survei->id) }}" method="POST"
                                    class="inline">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit"
